...
#' @param phase_colors Named vector of colors for each phase. Default uses four predefined hex codes.
...
createMtbpPhasesTracksWithRepliseq <- function(
  ...
  ht <- HighlightTrack(
    ...
    chromosome = chromosome,
    fill = rep("transparent", 3), col = c("#ED6A5A"),
